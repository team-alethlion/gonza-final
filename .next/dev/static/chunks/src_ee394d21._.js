            ...
            const calculateReconciliation = {
                "StockReconciliation.useEffect.calculateReconciliation": async ()=>{
                    ...
                    try {
                        ...
                        const { data: firstEntry } = await supabase.from('stock_history').select('new_quantity, created_at, change_reason').in('product_id', historyProductIds).eq('location_id', currentBusiness.id).order('created_at', {
                            ascending: true
                        }).order('id', {
                            ascending: true
                        }).limit(1).maybeSingle();
                        ...
                        while(hasSalesMore){
                            const { data: salesChunk, error: salesError } = await supabase.from('sales').select('items, date, receipt_number').eq('location_id', currentBusiness.id).range(salesStart, salesStart + chunkSize - 1).order('date', {
                                ascending: true
                            });
                        ...
                        while(hasHistoryMore){
                            const { data: chunk, error: historyError } = await supabase.from('stock_history').select('*').eq('location_id', currentBusiness.id).in('product_id', historyProductIds).order('created_at', {
                                ascending: true
                            }).order('id', {
                                ascending: true
                        ...
                        const { data: productData } = await supabase.from('products').select('quantity').eq('id', product.id).single();
...
const StockReconciliationPage = ()=>{
    ...
    const { data: allProducts = [], isLoading } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$tanstack$2f$react$2d$query$2f$build$2f$modern$2f$useQuery$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useQuery"])({
        ...
        queryFn: {
            "StockReconciliationPage.useQuery": async ()=>{
                ...
                let hasMore = true;
                while(hasMore){
                    const { data: chunk, error } = await supabase.from('products').select('*').eq('user_id', user.id).eq('location_id', currentBusiness.id).order('created_at', {
                        ascending: false
                    }).order('id', {
                        ascending: false
    ...
    const calculateProductReconciliation = async (product)=>{
        ...
        try {
            ...
            let hasSalesMore = true;
            while(hasSalesMore){
                const { data: salesChunk } = await supabase.from('sales').select('items, date, receipt_number').eq('location_id', currentBusiness.id).range(salesStart, salesStart + chunkSize - 1).order('date', {
                    ascending: true
                });
                if (salesChunk && salesChunk.length > 0) {
                    allSalesData.push(...salesChunk);
            ...
            let hasHistoryMore = true;
            while(hasHistoryMore){
                let query = supabase.from('stock_history').select('*').eq('location_id', currentBusiness.id).order('created_at', {
                    ascending: true
                }).order('id', {
                    ascending: true
                }).range(historyStart, historyStart + chunkSize - 1);
...
//# sourceMappingURL=src_ee394d21._.js.map