...
const useCarriageInwards = ()=>{
    ...
    const loadCarriageInwards = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useCarriageInwards.useCallback[loadCarriageInwards]": async ()=>{
            try {
                ...
                setIsLoading(true);
                const { data, error } = await supabase.from('carriage_inwards').select('*').eq('location_id', currentBusiness.id).order('date', {
                    ascending: false
                }).order('created_at', {
                    ascending: false
                });
    ...
    const createCarriageInward = async (data)=>{
        try {
            ...
            if (data.cashAccountId) {
                const { data: cashTransaction, error: cashError } = await supabase.from('cash_transactions').insert({
                    user_id: user.id,
            ...
            const { data: carriageData, error } = await supabase.from('carriage_inwards').insert({
                user_id: user.id,
                location_id: currentBusiness.id,
    ...
    const updateCarriageInward = async (id, updates)=>{
        try {
            ...
            const { error } = await supabase.from('carriage_inwards').update(updateData).eq('id', id).eq('location_id', currentBusiness.id);
            if (error) throw error;
            await loadCarriageInwards();
            toast({
                title: "Success",
    ...
    const deleteCarriageInward = async (id)=>{
        try {
            ...
            const { data: record, error: fetchError } = await supabase.from('carriage_inwards').select('cash_transaction_id').eq('id', id).eq('location_id', currentBusiness.id).single();
            if (fetchError) throw fetchError;
            // Delete associated cash transaction if exists
            if (record.cash_transaction_id) {
                const { error: cashError } = await supabase.from('cash_transactions').delete().eq('id', record.cash_transaction_id).eq('location_id', currentBusiness.id);
                if (cashError) throw cashError;
            }
            // Delete the carriage inwards record
            const { error } = await supabase.from('carriage_inwards').delete().eq('id', id).eq('location_id', currentBusiness.id);
            if (error) throw error;
            await loadCarriageInwards();
            toast({
                title: "Success",
...
//# sourceMappingURL=src_1579d23f._.js.map