...
export async function sendVerificationEmail(email: string, code: string) {
  ...
  try {
    ...
    const apiEndpoint = provider === 'smtp' ? '/api/email/smtp' : '/api/email/resend';
...
export async function sendSubscriptionNotificationEmail(email: string, details: { 
  userName: string,
  planName: string, 
  status: string, 
  expiryDate: string,
  ...
  const payload = {
    to: email,
    subject: `Subscription ${details.isTrial ? 'Trial Activated' : 'Confirmed'} - Gonza Systems`,
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 0; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; color: #333;">
        <div style="background: #252861; padding: 30px 20px; text-align: center;">
          <h1 style="color: #ffffff; margin: 0; font-size: 24px; letter-spacing: 1px;">Subscription ${details.isTrial ? 'Trial Activated' : 'Confirmed'}</h1>
        </div>
        
        <div style="padding: 30px; line-height: 1.6;">
          <p style="font-size: 18px; margin-top: 0;">Hello <strong>${details.userName}</strong>,</p>
          <p>Your subscription to the <strong>${details.planName}</strong> plan has been successfully ${details.isTrial ? 'activated as a free trial' : 'confirmed'}. You now have full access to Gonza Systems.</p>
          ...
          <div style="background: #f8f9fa; border-left: 4px solid #f05a2b; padding: 20px; margin: 25px 0; border-radius: 4px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                ...
                <td style="padding: 8px 0; font-weight: bold; color: #f05a2b;">${details.expiryDate}</td>
              </tr>
            </table>
          </div>
          ...
          <ul style="list-style: none; padding: 0; margin: 0;">
            <li style="margin-bottom: 10px; display: flex; align-items: center;">
              ...
              <span>Up to <strong>${details.limits.users}</strong> Team Members</span>
            </li>
            <li style="margin-bottom: 10px; display: flex; align-items: center;">
              <span style="color: #28a745; margin-right: 10px;">✔</span> 
              <span>Up to <strong>${details.limits.products}</strong> Products</span>
            </li>
            <li style="margin-bottom: 10px; display: flex; align-items: center;">
              <span style="color: #28a745; margin-right: 10px;">✔</span> 
              <span>Up to <strong>${details.limits.sales}</strong> Sales per Month</span>
            </li>
          </ul>

          <div style="text-align: center; margin-top: 40px;">
            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/agency" 
