...
import { getProductStatsAction, getFilteredProductsForExportAction } from '@/app/actions/products';
...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const loadTotals = async () => {
      ...
      if (!cancelled) {
        ...
        try {
          localStorage.setItem(cacheKey, JSON.stringify({ ...stats, ts: Date.now() }));
        } catch { }
      }
    };
    loadTotals();
  ...
  const fetchAllProductsForExport = async (): Promise<Product[]> => {
    if (!user?.id || !currentBusiness?.id) return [];
    try {
      const raw = await getFilteredProductsForExportAction(currentBusiness.id, filters);
      if (!raw) return [];
      const { mapDbProductToProduct } = await import('@/types');
      let formattedProducts = (raw as any[]).map(mapDbProductToProduct);
      ...
      return formattedProducts.sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'itemNumber': cmp = a.itemNumber.localeCompare(b.itemNumber); break;
        }
        return sortOrder === 'asc' ? cmp : -cmp;
      });
    } catch (error) {
      console.error('Error fetching all products for export:', error);
      toast({ title: 'Export failed', description: 'Failed to fetch products for export.', variant: 'destructive' });
      return [];
    }
  };
