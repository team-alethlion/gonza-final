    ...
    <div className="min-h-screen bg-[#fafafa] font-geist selection:bg-primary/10 overflow-x-hidden pb-12">
      <header className="h-16 border-b border-border/40 bg-white/80 backdrop-blur-md sticky top-0 z-20 flex items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push(`/admin/records/${id}/edit`)}
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <div>
              <h2 className="text-2xl font-bold text-foreground tracking-tight">
                {user?.business_name || "Account Setup"}
              </h2>
              <div className="flex items-center gap-3 mt-1.5">
                <span
                  className={`text-[8px] px-2 py-0.5 rounded font-bold uppercase tracking-widest ${
                    user?.is_frozen
          ...
          <div className="flex flex-col md:flex-row gap-6 lg:gap-8 items-start">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-3 mb-1">
                <h2 className="text-xl lg:text-2xl font-bold tracking-tight text-foreground truncate">
                  {user.business_name || "New User Account"}
                </h2>
              ...
              <p className="text-muted-foreground text-[10px] lg:text-[11px] font-bold uppercase tracking-widest truncate">
                {user.email}
              </p>
            </div>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-5">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-tight mb-1">
                ...
                <div className="space-y-5">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-medium text-foreground uppercase tracking-wider">
                      {user.location_count} Locations Discovered
                    </span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-tight mb-1">
                    ...
                    <span className="text-sm font-medium text-foreground">
                      {user.locations?.filter((l) => l.completed).length}{" "}
                      Verified Branches
                    </span>
                  </div>
                  <div className="flex flex-col pt-4 border-t border-border/20">
                    <span className="text-[9px] font-bold text-blue-600/50 uppercase tracking-tight mb-1">
                      Location Multiplier Limit
                    </span>
                    <span className="text-[11px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded w-fit uppercase tracking-widest">
                      {user.location_limit} Allowed Units
                    </span>
                  </div>
                </div>
              </div>
            ...
            {user.billing_amount > 0 || user.days_remaining > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-0 bg-white border border-border/40 rounded-xl overflow-hidden divide-x divide-border/30">
                <div className="px-5 py-5">
                  <span className="text-[9px] font-bold text-muted-foreground/50 uppercase tracking-tight block mb-1">
                    Plan Amount
                  </span>
                  <span className="text-sm font-bold text-foreground">
                    UGX {user.billing_amount.toLocaleString()}
                  </span>
                  <span className="text-[9px] text-muted-foreground block mt-0.5">
                    {user.billing_duration}
                  </span>
                </div>
                <div className="px-5 py-5">
                ...
                <div className="px-5 py-5">
                  ...
                  {user.days_remaining <= 0 ? (
                    <span className="text-[9px] font-bold bg-red-500/10 text-red-600 px-2 py-0.5 rounded uppercase tracking-tighter">
                      Expired
                    </span>
                  ) : user.days_remaining <= 7 ? (
                    <span className="text-[9px] font-bold bg-amber-500/10 text-amber-600 px-2 py-0.5 rounded uppercase tracking-tighter">
                      Expiring Soon
                    </span>
                  ) : (
                    <span className="text-[9px] font-bold bg-green-500/10 text-green-600 px-2 py-0.5 rounded uppercase tracking-tighter">
                      Active
                    </span>
                  )}
                </div>
              ...
              <p className="text-[11px] text-muted-foreground italic">
                No subscription configured for this account.
              </p>
            )}
          </div>
                ...
                <table className="w-full text-left border-collapse min-w-[500px]">
                  <thead>
                    <tr className="bg-[#fafafa] border-b border-border/40 h-10">
                      <th className="px-5 text-[9px] font-bold text-muted-foreground uppercase tracking-widest w-1/4">
                        Branch Name
                      </th>
                      <th className="px-5 text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                        Address
                      </th>
                      <th className="px-5 text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                        Contact Number
                      ...
                      <th className="px-5 text-[9px] font-bold text-muted-foreground uppercase tracking-widest w-40 text-right">
                        Status
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/20">
                    {user.locations?.map((loc) => (
                      <tr
                        key={loc.id}
                        className="h-12 hover:bg-[#fafafa]/50 transition-colors">
                        <td className="px-5 py-3">
                          <div className="flex flex-col">
                            <span className="text-[11px] font-bold text-foreground mb-0.5">
                              {loc.name}
                            </span>
                        ...
                        <td className="px-5 py-3 text-right">
                          {loc.completed ? (
                            <span className="text-[8px] font-bold bg-green-500/10 text-green-600 px-1.5 py-0.5 rounded uppercase tracking-tighter">
                              Verified
                            </span>
                          ) : (
                            <span className="text-[8px] font-bold bg-amber-500/10 text-amber-600 px-1.5 py-0.5 rounded uppercase tracking-tighter">
                              Draft
                            </span>
                          )}
                        </td>
