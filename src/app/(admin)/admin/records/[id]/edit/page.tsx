...
export default function EditRecord() {
  ...
  const [formData, setFormData] = useState<
    Partial<OnboardingRecord & { location_limit?: number; is_frozen?: boolean }>
  >({});
  ...
  const updateMutation = useMutation({
    mutationFn: async (
      updatedData: Partial<
        OnboardingRecord & { location_limit?: number; is_frozen?: boolean }
      >,
    ) => {
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `CAUTION: Are you sure you want to PERMANENTLY delete the account for "${user?.business_name}"? This action cannot be undone.`,
      )
    ) {
      return;
    }
          ...
          <form onSubmit={handleSave} className="space-y-8 lg:space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
              <div className="space-y-1.5">
                <label className="text-[9px] uppercase tracking-widest font-bold text-muted-foreground/60 ml-0.5">
                  Commercial Name
                </label>
                <input
                  type="text"
            ...
            <div className="pt-8 lg:pt-10 border-t border-border/40">
              <h3 className="text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground mb-6 lg:mb-8">
                Communication Channels
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase tracking-widest font-bold text-muted-foreground/60 ml-0.5">
                    Business Network Email
                  </label>
                  <input
                    type="email"
            ...
            <div className="pt-8 lg:pt-10 border-t border-border/40">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground/60 ml-0.5">
                    Days To Renewal
                  </label>
                  <div className="relative">
                    <input
                      type="text"
                      value={
                        formData.days_remaining !== undefined
                          ? formData.days_remaining
                          : ""
                      }
                      onChange={(e) => {
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase tracking-widest font-bold text-muted-foreground/60 ml-0.5">
                    Committed Amount (UGX)
                  </label>
                  <input
                    type="text"
                    value={
                      formData.billing_amount !== undefined
                        ? formData.billing_amount.toLocaleString()
                        : ""
                    }
                    onChange={(e) => {
                      const rawValue = e.target.value.replace(/,/g, "");
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        billing_duration: e.target.value as any,
                      })
                    }
              ...
              <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase tracking-widest font-bold text-blue-600/80 ml-0.5">
                    Location Multiplier Limit
                  </label>
                  <input
                    type="text"
                    value={
                      formData.location_limit !== undefined
                        ? formData.location_limit.toLocaleString()
                        : ""
                    }
                    onChange={(e) => {
                      const rawValue = e.target.value.replace(/,/g, "");
                ...
                <div className="space-y-3">
                  <label className="text-[9px] uppercase tracking-widest font-bold text-red-600/80 ml-0.5">
                  ...
                  <div className="flex items-center gap-3 bg-red-50/10 border border-red-200/50 p-3 rounded-lg">
                    ...
                    />
                    <label
                      htmlFor="freeze_account"
                      className="text-[10px] font-bold text-red-700 uppercase tracking-wider cursor-pointer select-none">
                      {formData.is_frozen
                        ? "Suspend Account Access (Frozen)"
