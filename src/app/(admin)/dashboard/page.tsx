...
interface LocationRecord {
    id: string; // business_locations.id
    onboarding_id: string | null; // business_onboarding.id
    name: string;
    address: string | null;
    completed: boolean;
...
interface UserSummary {
    ...
    is_frozen: boolean;
    created_at: string;
    // Billing fields (matching ShowRecord.tsx)
    billing_amount: number;
    billing_duration: string;
    days_remaining: number;
    next_billing_date: string;
...
export default function Dashboard() {
    ...
    const { data: users, isLoading } = useQuery<UserSummary[]>({
        ...
        queryFn: async () => {
            ...
            // The RPC now returns a single JSONB object which is the array
    ...
    const filteredUsers = users?.filter(u => {
        ...
        // Filter by aggregate status: 
        // 1. 'verified' if they have any completed location
        // 2. 'provisioning' if they have NO completed locations or NO locations at all (new sign-up)
    ...
    const handleAction = (type: 'edit' | 'delete' | 'freeze' | 'unfreeze', target: UserSummary) => {
    ...
    const executeAction = async () => {
        ...
        try {
            ...
            if (confirmModal.type === 'freeze' || confirmModal.type === 'unfreeze') {
                const { error } = await supabase.rpc('toggle_user_freeze', {
                    p_username: username,
                    p_password: password,
                    p_user_id: confirmModal.targetId,
                    p_is_frozen: confirmModal.type === 'freeze'
                });

                if (error) throw error;
                toast.success(`User account ${confirmModal.type === 'freeze' ? 'Frozen' : 'Unsuspended'} Successfully`);
                ...
                toast.info(`Action ${confirmModal.type} executed for ${confirmModal.targetName}`);
            }
        } catch (err: any) {
        ...
        setConfirmModal({ show: false, type: 'edit', targetId: null, targetName: null });
    };

    const exportCSV = () => {
        if (!filteredUsers) return;
        const headers = [
            'Owner Email', 'Primary Business', 'Contact Number',
            'Location Count', 'Status', 'Billing Amount (UGX)',
            'Billing Cycle', 'Days Remaining', 'Next Billing Date', 'Joined Date'
        ];
        const rows = filteredUsers.map(u => {
            ...
            return [
                ...
                primary?.next_billing_date ? format(new Date(primary.next_billing_date), 'yyyy-MM-dd') : '',
    ...
    const exportPDF = async () => {
        ...
        try {
            ...
            pdf.text(`Generated on ${format(new Date(), 'MMMM dd, yyyy HH:mm')}`, 14, 30);
            ...
            const tableData = filteredUsers.map(u => {
                ...
                return [
                    ...
                    primary?.billing_amount ? `UGX ${primary.billing_amount.toLocaleString()}` : '—',
                    primary?.billing_duration ?? '—',
                    primary?.days_remaining != null ? String(primary.days_remaining) : '—',
                    primary?.next_billing_date ? format(new Date(primary.next_billing_date), 'MMM dd, yyyy') : '—',
            ...
            autoTable(pdf, {
                startY: 40,
                head: [['Email', 'Business', 'Contact', 'Locs', 'Status', 'Billing Amt', 'Cycle', 'Days Left', 'Next Billing', 'Joined']],
                body: tableData,
                theme: 'striped',
            ...
            <aside className={cn(
                ...
                <div className="h-16 flex items-center px-6">
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs font-bold tracking-widest uppercase text-foreground">Console</span>
                    </div>
                </div>

                <div className="px-4 py-2">
                    <p className="px-3 mb-2 text-[10px] font-bold text-muted-foreground/50 uppercase tracking-[0.2em]">Management</p>
                ...
                <div className="mt-auto p-4 border-t border-border/40 bg-[#fafafa]/50">
                    <div className="flex items-center gap-3 px-3 py-2 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-bold text-foreground truncate uppercase tracking-tight">{user?.username}</p>
                            <div className="flex items-center gap-1">
                                <div className="w-1 h-1 rounded-full bg-green-500" />
                                <span className="text-[9px] text-muted-foreground uppercase font-bold tracking-widest">Online</span>
                            </div>
                        </div>
                    </div>
                    <button
            ...
            <main className="flex-1 lg:pl-56 w-full">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <h1 className="text-[10px] font-bold text-foreground uppercase tracking-[0.2em]">Dashboard</h1>
                            </div>

                            {/* Mobile Actions (PDF/CSV) - Small icons for mobile */}
                            <div className="flex items-center gap-1 lg:hidden">
                                <button onClick={exportPDF} className="p-2 hover:bg-muted rounded transition-colors text-muted-foreground">
                                    <div className="text-[8px] font-bold uppercase tracking-tighter">PDF</div>
                                </button>
                                <button onClick={exportCSV} className="p-2 hover:bg-muted rounded transition-colors text-muted-foreground">
                                    <div className="text-[8px] font-bold uppercase tracking-tighter">CSV</div>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                            ...
                            <div className="hidden lg:flex items-center gap-2 bg-white border border-border/40 rounded-[5px] p-1">
                                <button onClick={exportPDF} className="p-1 px-2.5 text-[9px] font-bold uppercase tracking-widest hover:bg-muted rounded transition-colors flex items-center gap-2">
                                    Export PDF
                                </button>
                                <button onClick={exportCSV} className="p-1 px-2.5 text-[9px] font-bold uppercase tracking-widest hover:bg-muted rounded transition-colors flex items-center gap-2">
                                    Export CSV
                                </button>
                            </div>
                        </div>
                ...
                <div className="p-4 lg:p-10 max-w-[1400px] mx-auto space-y-6 lg:space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            { label: 'Platform Users', value: users?.length || 0, icon: User, color: 'text-primary' },
                            { label: 'Total Locations', value: users?.reduce((acc, u) => acc + u.location_count, 0) || 0, icon: Building2, color: 'text-green-600' },
                            { label: 'Frozen Accounts', value: users?.filter(u => u.is_frozen).length || 0, icon: Snowflake, color: 'text-blue-500' }
                        ].map((stat) => (
                            <div key={stat.label} className="linear-card p-6 bg-white group hover:border-primary/20">
                                <div className="flex justify-between items-start mb-4">
                                    <p className="text-[9px] uppercase tracking-[0.15em] font-bold text-muted-foreground opacity-60">{stat.label}</p>
                                    <div className={cn("p-1.5 rounded bg-muted/30 transition-colors", stat.color)}>
                        ...
                        <div className="px-4 lg:px-6 py-3 border-b border-border/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-muted/[0.15]">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] font-bold text-foreground uppercase tracking-widest">Filters</span>
                                </div>
                                <div className="flex flex-wrap gap-1">
                                    {(['all', 'verified', 'provisioning'] as const).map((status) => (
                                        <button
                                            key={status}
                            ...
                            <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-[0.1em]">{filteredUsers?.length || 0} Managed Users</span>
                        </div>

                        <div className="overflow-x-auto" id="records-table-container">
                            <table className="w-full text-left min-w-[800px]" id="records-table">
                                <thead>
                                    <tr>
                                    ...
                                    {isLoading ? (
                                        <tr>
                                            <td colSpan={7} className="px-6 py-20 text-center">
                                                ...
                                                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Aggregating User Data...</p>
                                            </td>
                                        </tr>
                                    ) : filteredUsers?.map((u) => {
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="font-bold text-foreground text-[11px] truncate uppercase tracking-tight">{u.email}</p>
                                                            <p className="text-[9px] text-muted-foreground uppercase font-bold tracking-[0.1em]">Owner</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="font-bold text-foreground text-[10px] uppercase tracking-tight truncate">{u.business_name || 'Individual Entity'}</p>
                                                            <p className="text-[8px] text-muted-foreground uppercase font-medium tracking-widest truncate">{u.location_count > 1 ? `${u.location_count} Branches` : 'Primary Node'}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-[10px] font-bold text-foreground uppercase tracking-tight">{u.business_phone || '—'}</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <div className="flex flex-col items-center gap-1">
                                                        <div className={cn(
                                                            ...
                                                            (u.days_remaining ?? 0) <= 0 ? "bg-red-50 text-red-600 border-red-100" :
                                                                (u.days_remaining ?? 0) <= 5
                                                                    ? "bg-amber-50 text-amber-600 border-amber-100"
                                                                    : "bg-green-50 text-green-600 border-green-100"
                                                        )}>
                                                            {(u.days_remaining ?? 0) <= 0 ? 'Expired' : `${u.days_remaining} Days`}
                                                        </div>
                                                        <span className="text-[8px] text-muted-foreground font-medium">
                                                            {u.next_billing_date ? format(new Date(u.next_billing_date), 'MMM dd') : 'No Date'}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3 text-right">
                                                    <div className="flex items-center justify-end gap-1">
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                handleAction(u.is_frozen ? 'unfreeze' : 'freeze', u);
                                                            }}
                                                            className={cn(
                                                                "p-1.5 hover:bg-muted rounded transition-colors",
                                                                u.is_frozen ? "text-blue-600" : "text-muted-foreground hover:text-amber-600"
                                                            )}
                                                            title={u.is_frozen ? "Unsuspend User" : "Suspend User Account"}
                                                        >
                                                            <Snowflake className={cn("w-3.5 h-3.5", u.is_frozen && "animate-pulse")} />
                                                        </button>
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                        ...
                        <div className="flex flex-col items-center text-center mb-6">
                            <div className={cn(
                                ...
                                confirmModal.type === 'delete' ? "bg-red-50 text-red-600 border border-red-100" :
                                    confirmModal.type === 'freeze' || confirmModal.type === 'unfreeze' ? "bg-blue-50 text-blue-600 border border-blue-100" : "bg-primary/5 text-primary border border-primary/10"
                            )}>
                                <TriangleAlert className="w-6 h-6" />
                            </div>
                            <h3 className="text-base font-bold tracking-tight text-foreground uppercase tracking-widest">
                                Confirm {confirmModal.type}
                            </h3>
                            <p className="text-[13px] text-muted-foreground mt-2 leading-relaxed">
                                Proceed with action for<br />
                                <span className="font-bold text-foreground">{confirmModal.targetName}</span>
                            </p>
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <button
                                onClick={() => setConfirmModal({ show: false, type: 'edit', targetId: null, targetName: null })}
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    confirmModal.type === 'delete' ? "bg-red-600 hover:bg-red-700" :
                                        confirmModal.type === 'freeze' || confirmModal.type === 'unfreeze' ? "bg-amber-600 hover:bg-amber-700" : "bg-primary hover:bg-primary/95"
                                )}
                            >
                                Confirm
                            </button>
