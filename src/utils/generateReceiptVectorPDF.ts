...
export const generateReceiptVectorPDF = async (
  ...
  currentY += addText(
    receiptData.documentTitle.toUpperCase(),
    pageWidth / 2,
    currentY,
    {
      fontSize: 16,
  ...
  leftY += addText(
    `${receiptData.documentNumberLabel} ${receiptData.receiptNumber}`,
    innerLeft,
    leftY,
    {
      fontSize: 11,
  ...
  if (receiptData.customerName && receiptData.customerName.trim() !== "") {
    ...
    if (
      ...
      receiptData.customerAddress.trim() !== ""
    ) {
      // Customer address without "Address:" label
      const addressHeight = addMultilineText(
        receiptData.customerAddress,
        innerLeft,
        currentY,
        innerWidth * 0.8,
    ...
    if (
      ...
      receiptData.customerContact.trim() !== ""
    ) {
  ...
  const hasDiscounts = receiptData.items.some(
    (item) =>
  ...
  receiptData.items.forEach((item, index) => {
    ...
    addText(
      ...
      tableX + colWidths[1] - 3,
      currentY + calculatedRowHeight / 2 + 1.5,
      {
        fontSize: 9,
        align: "right",
      },
    ...
    addText(
      formatCurrency(itemTotal, receiptData.currency),
      tableX + colWidths[finalColumnIndex] - 3,
      currentY + calculatedRowHeight / 2 + 1.5,
      {
        fontSize: 9,
        align: "right",
  ...
  if (totalDiscount > 0) {
    ...
    addText(
      formatCurrency(subtotal + totalDiscount, receiptData.currency),
      totalsStartX + labelWidth + valueWidth - 4,
      currentY + 5.5,
      {
        fontSize: 10,
        fontStyle: "normal",
    ...
    addText(
      `-${formatCurrency(totalDiscount, receiptData.currency)}`,
      totalsStartX + labelWidth + valueWidth - 4,
      currentY + 5.5,
      {
        fontSize: 10,
  ...
  addText(
    totalDiscount > 0 ? "Subtotal (after discount):" : "Subtotal:",
    totalsStartX + 4,
    currentY + 5.5,
    {
      fontSize: 10,
  ...
  addText(
    formatCurrency(subtotal, receiptData.currency),
    totalsStartX + labelWidth + valueWidth - 4,
    currentY + 5.5,
    {
      fontSize: 10,
  ...
  if (receiptData.showTaxRow && (receiptData.taxAmount || 0) > 0) {
    ...
    addText(
      formatCurrency(receiptData.taxAmount || 0, receiptData.currency),
      totalsStartX + labelWidth + valueWidth - 4,
      currentY + 5.5,
      {
        fontSize: 10,
  ...
  addText(
    formatCurrency(receiptData.totalAmount, receiptData.currency),
    totalsStartX + labelWidth + valueWidth - 4,
    currentY + 7.5,
    {
      fontSize: 12,
  ...
  if (receiptData.isInstallmentSale) {
    ...
    if (receiptData.amountPaid !== undefined) {
      ...
      addText(
        formatCurrency(receiptData.amountPaid, receiptData.currency),
        totalsStartX + labelWidth + valueWidth - 4,
        currentY + 5.5,
        {
          fontSize: 10,
    ...
    if (receiptData.amountDue !== undefined) {
      ...
      addText(
        formatCurrency(receiptData.amountDue, receiptData.currency),
        totalsStartX + labelWidth + valueWidth - 4,
        currentY + 5.5,
        {
          fontSize: 10,
  ...
  if (receiptData.totalAmountInWords) {
    ...
    const wordsHeight = addMultilineText(
      amountText,
      innerLeft,
      currentY,
      innerWidth * 0.9,
  ...
  if (
    ...
    const validPayments = receiptData.paymentMethods.filter(
      (payment) =>
        payment.method.trim() ||
  ...
  if (receiptData.signature) {
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        img.src = receiptData.signature as string;
      });
    } catch (error) {
