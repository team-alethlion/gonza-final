...
export const useTaskDraft = () => {
  ...
  const DRAFT_STORAGE_KEY = useMemo(() =>
    currentBusiness?.id ? `task_draft_${currentBusiness.id}` : 'task_draft'
    , [currentBusiness]);

  const SESSION_BACKUP_KEY = useMemo(() =>
    currentBusiness?.id ? `task_backup_session_${currentBusiness.id}` : 'task_backup_session'
    , [currentBusiness]);

  const [hasDraft, setHasDraft] = useState(() => {
    if (typeof window === 'undefined') return false;
    const businessId = currentBusiness?.id;
    const persistentKey = businessId ? `task_draft_${businessId}` : 'task_draft';
    const sessionKey = businessId ? `task_backup_session_${businessId}` : 'task_backup_session';
  ...
  const loadDraft = useCallback(() => {
    ...
    if (draft) {
      try {
        ...
        console.error('Error parsing task draft:', error);
