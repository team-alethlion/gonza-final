...
export const useCarriageDraft = () => {
  const { currentBusiness } = useBusiness();

  const DRAFT_STORAGE_KEY = useMemo(
    () =>
      currentBusiness?.id
        ? `carriage_draft_${currentBusiness.id}`
        : "carriage_draft",
    [currentBusiness],
  );
  ...
  const SESSION_BACKUP_KEY = useMemo(
    () =>
      currentBusiness?.id
        ? `carriage_backup_session_${currentBusiness.id}`
        : "carriage_backup_session",
    [currentBusiness],
  );
  ...
  const [hasDraft, setHasDraft] = useState(() => {
    ...
    const persistentKey = businessId
      ? `carriage_draft_${businessId}`
      : "carriage_draft";
    const sessionKey = businessId
      ? `carriage_backup_session_${businessId}`
      : "carriage_backup_session";
    return (
  ...
  const loadDraft = useCallback(() => {
    ...
    if (draft) {
      try {
        ...
        console.error("Error parsing carriage draft:", error);
