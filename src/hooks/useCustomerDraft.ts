...
export const useCustomerDraft = () => {
  const { currentBusiness } = useBusiness();

  const DRAFT_STORAGE_KEY = useMemo(
    () =>
      currentBusiness?.id
        ? `customer_draft_${currentBusiness.id}`
        : "customer_draft",
    [currentBusiness],
  );
  ...
  const SESSION_BACKUP_KEY = useMemo(
    () =>
      currentBusiness?.id
        ? `customer_backup_session_${currentBusiness.id}`
        : "customer_backup_session",
    [currentBusiness],
  );
  ...
  const [hasDraft, setHasDraft] = useState(() => {
    ...
    const persistentKey = businessId
      ? `customer_draft_${businessId}`
      : "customer_draft";
    const sessionKey = businessId
      ? `customer_backup_session_${businessId}`
      : "customer_backup_session";
    return (
  ...
  const loadDraft = useCallback(() => {
    ...
    if (draft) {
      try {
        ...
        console.error("Error parsing customer draft:", error);
